...
app.get('/book_dtls', async (req, res) => {
  try {
    const book = await Book.findById(req.query.id).populate('author').populate('genre');
    if (!book) {
...
app.post('/newbook', async (req, res) => {
  ...
  try {
    let author = await Author.findOne({ family_name: familyName, first_name: firstName });
    if (!author) {
      author = new Author({ first_name: firstName, family_name: familyName });
      await author.save();
    }
